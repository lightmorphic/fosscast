...
test('a handle-style address survives, which is what people arrive with', () => {
  ...
  assert.strictEqual(aliases.tidy('https://example.org/@a-show/feed.xml'), '/@a-show/feed.xml');
  assert.strictEqual(aliases.tidy('/@a-show/feed.xml'), '/@a-show/feed.xml');
...
test('saving an old address leaves the rest of the show alone', async () => {
  ...
  const screen = fs.readFileSync(path.join(__dirname, '..', 'lib', 'admin', 'podcast-page.js'), 'utf8');
  const form = screen.slice(screen.indexOf('id="sec-old-addresses"'));

...
test('any shape is welcome, so long as it covers the strip', () => {
  ...
  assert.strictEqual(bannerVideoProblem({ width: 1280, height: 720, duration: 6, bytes: 900 * 1024 }), null, '16:9');
...
test('too small to fill the strip is refused, and says by how much', () => {
  ...
  assert.match(problem, /1280 x 320/, 'the size it has to clear');
  assert.match(problem, /keep the shape/i, 'and that the shape is not the problem');
});

...
| Banner video | **shape unchanged, at least 1000 x 250** | Optional, alongside the still banner: with both uploaded you choose the video, the still, or either at random on each visit. Do not crop it: shrink it keeping its own shape until the smaller side just clears 1000 x 250 - the size the banner is actually drawn at - then choose which part shows by dragging a box over the playing video in the dashboard. Aim for under 1 MB, 5-8 seconds, about 1 Mbps, no audio; refused past 1920 x 1080, 1.5 MB, 10 seconds or 1.5 Mbps. Nothing is re-encoded on the server, so a video is served exactly as uploaded. |
